...
기본 목록이 있는 엔진은 여기에서 `<engine>` 태그를 지원 합니다: * `cordova` * `cordova-plugman` * `cordova-android` * `cordova-ios` * `cordova-blackberry10` * `cordova-wp7` * `cordova-wp8` *`cordova-windows8`  
* `android-sdk` / 설치 수준 높은 안 드 로이드 api 반환 합니다 / * `apple-xcode` / xcode 버전 반환 합니다 / * `apple-ios` / / 설치 된 가장 높은 iOS 버전을 반환 합니다 * `apple-osx` / OSX 버전 반환 합니다 / * `blackberry-ndk` / / 네이티브 blackberry SDK 버전을 반환 합니다 * `windows-os` / / 윈도우 OS 버전을 반환 합니다 * `windows-sdk` / / MSBuild 버전을 반환 합니다
