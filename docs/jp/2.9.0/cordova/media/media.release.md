...
`media.release` メソッドは同期的に OS のオーディオリソースを開放します。 Android ではメディア再生のための OpenCore インスタンスが有限のため、この関数は特に Android にとって重要な関数です。 `Media` リソースが不要になった場合には、 `release` メソッドを使用してください。
