...
キャプチャー操作は、ユーザーがビデオ録画アプリを終了するか、 `CaptureVideoOptions.limit` で指定された最大録画回数に達した場合に終了します。もし `limit` パラメーターが指定されていない場合は、デフォルト値である1が使用され、キャプチャー操作はユーザーが1度ビデオを録画した後に終了します。
