...
デバイスが、 `geolocationOptions` の `timeout` プロパティーによって指定された時間内に位置が取得できなかった場合に返されます。 `geolocation.watchPosition` と一緒に使用するとき、`geolocationError` コールバックが `timeout` ミリ秒ごとに呼び出される可能性があります。
