...
To lahko traja nekaj časa za ukaz za dokončanje, zato bodite potrpežljivi. Teči s `cordova -d` informacije o napredku.

Prvi argument določa *hello* mapo za vaš projekt. Svoje `www` subdirectory hiše aplikacije domačo stran, skupaj z različnih virov pod `css` , `js` , in `img` , ki sledijo skupne spletne razvoj poimenovanja datotek. Na `config.xml` Datoteka vsebuje pomembno metapodatkov, potrebne za ustvarjanje in distribucijo uporabo.

Drugih dveh trditev so neobvezne: je `com.example.hello` argument določa vaš projekt z identifikatorjem povratne domene slogu, in `HelloWorld` zagotavlja besedilo za prikaz programa. Urejate lahko oba te vrednosti kasneje v v `config.xml` datoteko.
...
Izvaja ukaze za dodajanje ali odstranjevanje platforme vpliva na vsebino projekta *platforme* imenik, kjer se zdi vsako določeno platformo kot podimenik. *Www* Izvorni imenik je povzeto v podimeniku vsako platformo, ki se pojavljajo na primer v `platforms/ios/www` ali `platforms/android/assets/www` . Privzeto, vsako platformo konfiguracijske datoteke nastavljen da lahko dostopate do vseh Cordova's API.

Če želite, lahko uporabljate je SDK na tej točki odpreti projekt, ki ste jo ustvarili. Popravljanje k projektu v SDK vplivajo na odvod določijo, sredstev, ne izvirne datoteke prek-plosčad vir. Ta pristop uporabite, če želite le inicializirati projekt. (Glej navodila platformo za informacije o tem, kako razviti aplikacije znotraj vsakega SDK.) Berite naprej, če želite uporabljati orodja ukazne vrstice za celoten razvoj ciklus.
...
Nekatere mobilne platforme tekmovati z posebno napravo privzeto, kot so iPhone iOS projektov. Za druge platforme, boste morda morali najprej povežite napravo z emulator. (Glej navodila platformo za podrobnosti.) Na primer, boste morda najprej zagnati v `android` ukaz za zagon Android SDK, nato zaženite določene naprave sliko, ki se začenja po svoje privzeto vedenje:
...
Preden zaženete ta ukaz, morate nastaviti napravo za testiranje, po postopkih, ki se razlikujejo za vsako platformo. V primeru Android, bi morali usposobiti **USB debugging** možnost na napravi, in morda dodali USB voznik glede na razvoj environmnent. Glej platformo vodniki za podrobnosti o zahtevah za vsako platformo.
...
Je `cordova plugin add` ukaz zahteva, da določite skladišča za plugin kodo. Tukaj so primeri funkcije lahko dodate:
