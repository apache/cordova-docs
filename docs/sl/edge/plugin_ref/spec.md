...
Tukaj je seznam privzeti iskalniki, ki je `<engine>` trakec zaslomba: * `cordova` * `cordova-plugman` * `cordova-android` * `cordova-ios` * `cordova-blackberry10` * `cordova-wp7` * `cordova-wp8` *`cordova-windows8`  
* `android-sdk` / / vrne najvišji Android api ravni nameščen * `apple-xcode` / / vrne xcode različico * `apple-ios` / / vrne najvišji nameščena različica iOS * `apple-osx` / / vrne različico OSX * `blackberry-ndk` / / vrne native blackberry različica SDK * `windows-os` / / vrne različico Windows OS * `windows-sdk` / / vrne MSBuild različica
...
Kot izvorne datoteke, vendar posebej za platforme, kot so iOS da razlikujete od izvorne datoteke, glave in virov. Primeri:
