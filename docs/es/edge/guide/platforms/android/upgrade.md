...
1.  Ejecute `bin/update <project_path>`.
...
3.  Copie el archivo `res/xml/config.xml` para que coincida con el `framework/res/xml/config.xml`.
...
6.  Añadir el `res/xml/plugins.xml` para que coincida con el `framework/res/xml/plugins.xml`.