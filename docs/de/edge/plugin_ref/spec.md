...
Hier ist eine Liste der Standard-Suchmaschinen, die den `<engine>` tag unterstützt: * `cordova` * `cordova-plugman` * `cordova-android` * `cordova-ios` * `cordova-blackberry10` * `cordova-wp7` * `cordova-wp8` *`cordova-windows8`  
* `android-sdk` / / liefert die höchste Android-api-Ebene installiert * `apple-xcode` / / liefert die Xcode-Version * `apple-ios` / / liefert die höchste installierte iOS-Version * `apple-osx` / / liefert die OSX Version * `blackberry-ndk` / / liefert die systemeigenen Blackberry SDK Version * `windows-os` / / liefert die Windows OS-Version * `windows-sdk` / / liefert die MSBuild-Version
