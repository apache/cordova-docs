...
Um den einheitlichen Teil Ihr Plugin zu erstellen, öffnen Sie die BlackBerry 10 NDK IDE, und wählen Sie **Datei → neu → BlackBerry Project → Native Erweiterung → BlackBerry WebWorks**. Geben Sie den gewünschten Projektnamen und den Speicherort, und drücken Sie **Fertig stellen**.
