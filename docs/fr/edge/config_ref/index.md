...
Plusieurs aspects du comportement de l'application peuvent être contrôlés avec un fichier de configuration globale, `config.xml` , qui appartient au répertoire actif web de niveau supérieur ainsi que de la page d'accueil de l'application. Ce fichier XML de plate-forme agnostique est organisé selon spécification [Emballés Web Applications (Widgets)][1] de la W3C et s'étend pour spécifier des caractéristiques de Cordova API de base, des plugins et des paramètres spécifiques à la plateforme.

 [1]: http://www.w3.org/TR/widgets/

Pour les projets créés avec la CLI Cordova (décrites dans l'Interface de ligne de commande), ce fichier se trouve dans le niveau supérieur `www` répertoire :

        app/www/config.xml
    

Lorsque vous utilisez l'interface CLI pour générer un projet, les versions de ce fichier sont copiées passivement ainsi que d'autres fichiers de source web-actif dans divers `platforms/*/www` sous-répertoires, par exemple :

        app/platforms/ios/www/config.xml
        app/platforms/blackberry10/www/config.xml
    

Une exception est Android, dont le chemin spécifie un autre `assets` sous-répertoire :

        App/plateformes/android/actifs/www/config.xml
    

Si vous utilisez la CLI pour créer un projet, mais ensuite déplacer votre flux de travail à un SDK, vous devez utiliser un autre ensemble de fichiers sources pour Android et iOS :
