...
ここでエンジンを既定の一覧は、 `<engine>` タグをサポートしています: * `cordova` * `cordova-plugman` * `cordova-android` * `cordova-ios` * `cordova-blackberry10` * `cordova-wp7` * `cordova-wp8` *`cordova-windows8`  
* `android-sdk` //インストール レベル最高の Android の api を返します * `apple-xcode` //xcode バージョンを返します。 * `apple-ios` //インストールされて最高の iOS のバージョンを返します。 * `apple-osx` //OSX のバージョンを返します。 * `blackberry-ndk` //ネイティブなブラックベリー SDK のバージョンを返します。 * `windows-os` //Windows OS のバージョンを返します。 * `windows-sdk` //MSBuild バージョンを返します。
