...
이러한 지침을 적어도 필요로 코르도바 2.3와 Xcode 4.5와 함께 한 `config.xml` 새로 만든된 iOS 프로젝트에서 파일. 절차 수 있습니다는 <a href="../../cli/index.html">명령줄 인터페이스</a>에서 새 프로젝트를 만든 다음 얻을 하는 `config.xml` 내에서 명명 된 응용 프로그램의 하위 디렉터리 내에서 파일`platforms/ios`.
