...
안 드 로이드에 익숙하지 당신은 먼저 안 드 로이드 <a href="../index.html">플랫폼 <a href="../../../index.html">가이드</a></a>를 숙지 해야 및 설치는 WebView를 포함의 더 특이 한 개발 옵션을 시도 하기 전에 최신 안 드 로이드 SDK가. 코르도바 1.9 부터는 안 드 로이드 플랫폼에 의존 한 `CordovaWebView` 는 레거시 빌드 구성 요소 `CordovaActivity` 사전 1.9 릴리스 날짜 구성 요소.
