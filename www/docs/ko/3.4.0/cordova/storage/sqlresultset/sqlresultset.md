...
때는 `[SQLTransaction](../sqltransaction/sqltransaction.html)` 개체의 `executeSql` 메서드가 호출 되어, 지정 된 콜백을 실행 한 `SQLResultSet` 매개 변수.
...
때는 `[SQLTransaction](../sqltransaction/sqltransaction.html)` 개체의 `executeSql` 메서드가 호출 되어, 지정 된 콜백 실행 한 `SQLResultSet` 세 가지 속성을 포함 하는 매개 변수:
