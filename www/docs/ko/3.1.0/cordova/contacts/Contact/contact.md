...
연락처를 사용자의 개인 또는 비즈니스 [연락처](../contacts.html) 등을 설명 하는 속성이 포함 되어 있습니다.
...
*   **별명**: 캐주얼 이름 [연락처](../contacts.html) 주소입니다. *(DOMString)*
...
*   **카테고리**: 모든 사용자 정의 범주 연락처에 [연결](../../connection/connection.html) 된 배열. *(ContactField[])*

*   **url**: 연락처에 [연결](../../connection/connection.html) 된 웹 페이지의 배열. *(ContactField[])*

## 메서드

*   **복제**: 새로운 반환 합니다 `Contact` 으로 호출 하는 개체의 전체 복사본은 개체는 `id` 속성으로 설정`null`.

*   **제거**: [장치](../../device/device.html) [연락처](../contacts.html) 데이터베이스에서 연락처를 제거 합니다, 그렇지 않으면와 오류 콜백을 실행 한 `[ContactError](../ContactError/contactError.html)` 개체.

*   **저장**: [장치](../../device/device.html) [연락처](../contacts.html) 데이터베이스를 새 연락처를 저장 또는 동일한 **id** 를 가진 연락처가 이미 있는 경우 기존 연락처를 업데이트 합니다.

## 세부 정보

`Contact`개체는 사용자의 연락처를 나타냅니다. [연락처](../contacts.html) 생성 수, 저장 또는 [장치](../../device/device.html) [연락처](../contacts.html) 데이터베이스에서 제거 합니다. 연락처도 검색할 수 있습니다 (개별적으로 또는 일괄적으로) 데이터베이스에서 호출 하 여는 `[contacts.find](../contacts.find.html)` 방법.

**참고:** 모든 [연락처](../contacts.html) 필드 위에 나열 된 모든 [장치](../../device/device.html) 플랫폼에서 지원 됩니다. 자세한 내용은 각 플랫폼의 *단점이* 섹션을 확인 하시기 바랍니다.
...
*   **카테고리**: 안 드 로이드 2.X [장치](../../device/device.html), 반환에서 지원 되지 않습니다`null`.
...
*   **사진**: 부분적으로 지원 합니다. 단일 축소판 그림 크기의 사진 지원 됩니다. 연락처의 사진으로 설정 하려면 전달에 base64 인코딩된 이미지 또는 이미지를 가리키는 URL. 블랙베리 [연락처](../contacts.html) 데이터베이스에 저장 하기 전에 이미지는 축소 된. [연락처](../contacts.html) 사진 base64 인코딩 이미지로 반환 됩니다.
...
*   **사진**: 응용 프로그램의 임시 디렉터리에 저장 된 이미지 [파일](../../file/fileobj/fileobj.html) URL을 반환 합니다. 응용 프로그램이 종료 될 때 임시 디렉터리의 내용은 제거 됩니다.

*   **카테고리**:이 속성은 현재 지원 되지 않습니다, 반환`null`.

## Windows Phone 7, 8 특수

*   **displayName**: 연락처를 만들 때 표시 이름에서 표시 이름 매개 [변수](../../../plugin_ref/spec.html) 다릅니다 제공 값 검색 연락처를 찾을 때.
