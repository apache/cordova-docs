...
[카메라](../camera.html) 설정을 사용자 지정 하는 선택적 매개 [변수](../../../plugin_ref/spec.html).
...
*   **품질**: 범위 0-100, 100은 [파일](../../file/fileobj/fileobj.html) 압축에서 손실 없이 일반적으로 전체 해상도 저장된 된 이미지의 품질. *(수)* (Note 카메라의 해상도 대 한 정보는 사용할 수 없습니다.)
...
*   **mediaType**:에서 선택 [미디어](../../media/media.html) 유형을 설정 합니다. 때에 작동 `PictureSourceType` 는 `PHOTOLIBRARY` 또는 `SAVEDPHOTOALBUM` . 에 정의 된 `nagivator.camera.MediaType` *(수)* 
...
*   **correctOrientation**: [캡처](../../media/capture/capture.html) 도중 장치의 방향에 대 한 해결 하기 위해 이미지를 회전 합니다. *(부울)*

*   **saveToPhotoAlbum**: [캡처](../../media/capture/capture.html) 후 장치에서 사진 앨범에 이미지를 저장 합니다. *(부울)*

*   **popoverOptions**: iPad에 popover 위치를 지정 하는 iOS 전용 옵션. 에 정의 된`[CameraPopoverOptions](CameraPopoverOptions.html)`.
...
*   어떤 `cameraDirection` 다시 [연결](../../connection/connection.html) 사진에 결과 값.

*   무시는 `allowEdit` 매개 [변수](../../../plugin_ref/spec.html).

*   `Camera.PictureSourceType.PHOTOLIBRARY`그리고 `Camera.PictureSourceType.SAVEDPHOTOALBUM` 둘 다 동일한 사진 앨범을 표시 합니다.

## 블랙베리 단점

*   무시는 `quality` 매개 [변수](../../../plugin_ref/spec.html).

*   무시는 `sourceType` 매개 [변수](../../../plugin_ref/spec.html).

*   무시는 `allowEdit` 매개 [변수](../../../plugin_ref/spec.html).

*   응용 프로그램 사용자 스냅 사진 후 네이티브 [카메라](../camera.html) 응용 프로그램을 닫으려면 키 삽입 권한이 있어야 합니다.

*   큰 이미지 크기를 사용 하 여 이미지를 인코딩하 나중 모델 [장치](../../device/device.html) (예를들면, 토치 9800)에 그 기능은 고해상도 [카메라](../camera.html) 무 능력에서 발생할 수 있습니다.

*   `Camera.MediaType`지원 되지 않습니다.

*   무시는 `correctOrientation` 매개 [변수](../../../plugin_ref/spec.html).

*   무시는 `cameraDirection` 매개 [변수](../../../plugin_ref/spec.html).

## iOS 단점

*   설정 `quality` 일부 [장치](../../device/device.html) 메모리 오류를 피하기 위해 50 아래.
...
*   항상 [파일](../../file/fileobj/fileobj.html) URI를 반환 합니다.
