...
플러그인은 `Plugin` Clsss를 상속받아야 한다. 플러그인은 **반드시** `execute` 메소드를 가져야하고 이 메소드는`PluginResult`객체를 리턴해야 한다. 그리고 플러그인은 <a href="../../../cordova/events/events.pause.html">pause</a>와 <a href="../../../cordova/events/events.resume.html">resume</a> 이벤트를 핸들링 할 수 있도록 하는 것이 좋으며 플러그인간에 전해지는 메세지를 핸들링할 수 있어야 한다.
