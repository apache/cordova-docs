...
대화형 함수에서 호출 된 `[pause](events.pause.html)` [이벤트](events.html) 처리기 실행 후 응용 프로그램 다시 시작 될 때에 의해 신호는 `resume` [이벤트](events.html). 이들은 경고, `console.log()` , 및 목표-c.를 통해가 플러그인 이나 API, 코르도바에서 호출
