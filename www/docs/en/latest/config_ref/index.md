...
AndroidEdgeToEdge<br/>{% cdv_vartype boolean %} {% cdv_platform android %} | *Default: false* <br/> Added in cordova-android 14.0.0 to enable Android edge-to-edge mode optionally. Since cordova-android 15.0.0 the app always runs in edge-to-edge mode, but introduced a compatibility layer that keeps status and navigation bars styled similarly to previous Android releases and positions the webview as it would have on older API levels, so users don't have to worry about refactoring their app to handle safe area insets. The status bar plugin `cordova-plugin-statusbar` is optional and must no be used anymore. The preference `StatusBarBackgroundColor` can still be used without the plugin. Also a Status Bar JavaScript API was added for setting the background color by `window.statusbar.setBackgroundColor()` and setting the visibility by `window.statusbar.visible`. The user can decide to opt into edge-to-edge or not, while Google will not claim about not using edge-to-edge. Using edge-to-edge will be a requirement by Google, when it will raise the minimum required target SDK to 36 (Android 16). Setting `true` will opt into full edge-to-edge rendering, where the status and bottom bar will be transparent and overlay the web view. You have to handle the safe area by the CSS variables [safe-area-inset-*](https://developer.mozilla.org/en-US/docs/Web/CSS/Guides/Environment_variables/Using#safe-area-inset-). The Status Bar JS API will not have any effect anymore when edge-to-edge is enabled.
