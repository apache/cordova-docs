...
AndroidEdgeToEdge<br/>{% cdv_vartype boolean %} {% cdv_platform android %} | *Default: false* <br/> Added in cordova-android 14.0.0 to enable Android edge-to-edge mode, which will be a requirement by Google when targeting Android 16 in a future release. Cordova uses a compatibility behavior that keeps status and navigation bars styled similarly to previous Android releases. Setting `true` will opt in to full edge-to-edge rendering. Using the Status Bar JS API, like `window.statusbar.visible` and `window.statusbar.setBackgroundColor()` will not have any effect if this setting is `true`.
