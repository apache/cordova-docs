...
SplashScreenBackgroundColor <br/> ==Android== ==Windows== | *Android default: #FFC8C8C8. Windows default: #464646.* <br/>  Sets the splashscreen background color. Supports a CSS color name or a four-byte hex value, with the first byte representing the alpha channel, and standard RGB values for the following three bytes. <br/> On Windows the alpha channel is ignored although `transparent` value will cause black/white background color in case of Dark/Light theme accordingly. You should put this preference under the platform tags to avoid applying a wrong color.
