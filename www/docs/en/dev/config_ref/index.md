...
InspectableWebview(boolean) <br/> ==iOS== | *Default: depends on Debug/Release build* <br/>  As of iOS 16.4 the webview is no longer inspectable by default. Cordova now allows inspection on Debug builds, but not on Release builds by default. Set this to true to force inspectability regardless of build settings.
