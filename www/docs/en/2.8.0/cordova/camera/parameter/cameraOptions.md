...
- Images saved to the photo album using the `saveToPhotoAlbum` parameter does not contain the same image metadata as compared to the image returned from <a href="../camera.getPicture.html">camera.getPicture</a>.  For example, the image saved to the photo album does not contain any GPS metadata.
