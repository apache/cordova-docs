...
- Images saved to the photo album using the `saveToPhotoAlbum` parameter does not contain the same image metadata as compared to the image returned from [camera.getPicture](../camera.getPicture.html).  For example, the image saved to the photo album does not contain any GPS metadata.
