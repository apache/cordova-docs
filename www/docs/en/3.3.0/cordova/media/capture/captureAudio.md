...
- Cordova for BlackBerry 10 attempts to launch the __Voice Notes Recorder__ application, provided by RIM, to capture audio recordings. The app receives a `<a href="capture.html">Capture</a>Error.CAPTURE_NOT_SUPPORTED` error code if the application is not installed on the device.
