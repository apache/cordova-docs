...
從 3.0 版本開始，科爾多瓦實現所有設備 Api 作為外掛程式，然後留在預設情況下禁用。 此外，它還支援兩種不同的方法來添加和刪除外掛程式，根據您的工作流<a href="../guide/overview/index.html">概述</a>中討論的選擇：

*   如果您使用跨平臺的工作流，則使用 `cordova` CLI 實用程式添加外掛程式，如所述的<a href="../guide/cli/index.html">命令列介面</a>。 CLI 一次修改所有指定平臺的外掛程式。

*   如果你使用的平臺為中心的工作流程，您使用一個較低級別的[Plugman][1]<a href="../guide/cli/index.html">命令列介面</a>，分別為每個目標平臺。
