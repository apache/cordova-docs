...
その対話型関数から呼び出された、 `<a href="events.pause.html">pause</a>` 後<a href="events.html">イベント</a> ハンドラーで実行、アプリケーションの再開時によってシグナル状態になると、 `resume` <a href="events.html">イベント</a>。 これらの警告を含める `console.log()` と目的 C. を通過するプラグインやコルドバ、API からの呼び出し
