...
*   `platform`(必須): あなたのフレームワークをサポートするプラットフォーム。 ワイルドカードを使用することがあります `*` すべての<a href="../guide/support/index.html">プラットフォームのサポート</a>されていることのようなパイプ文字で複数指定 `android|ios|blackberry10` またはちょうどのような単一のプラットフォーム`android`.
